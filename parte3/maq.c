...
void exec_maquina(Maquina *m, int timestep) {/*Excecuta as instrucoes de uma
  ...
  for (i = 0; i < timestep; i++) {
	...
	arg.t = prg[ip].op.t;
	if (prg[ip].op.t == NUM)
		arg.val.n = prg[ip].op.val.n;
	else if (prg[ip].op.t == VAR)
		arg.val.v = prg[ip].op.val.v;
	else
		arg.val.cel = prg[ip].op.val.cel;
	...
	switch (opc) {
	  ...
	  case POP:/*Nao necessita verificacao de tipo; retira o objeto do topo*/
	    desempilha(pil);
	    break;
	  case DUP:/*Nao necessita de verificacao de tipo; duplica o objeto do 
	  			topo*/
	    tmp = desempilha(pil);
	    empilha(pil, tmp);
	    empilha(pil, tmp);
	  ...
	  case JIF:/*Verifica o tipo do topo da pilha e da o jump para o argumento
	    ...
	    break;
	  case CALL:/*Empilha na pilha de excecucao o valor de rbp e de ip atuais 
	  			e da os novos valores de rbp e de ip a serem usados na pilha de
				excecucao*/
	    if (arg.t == NUM) {
	  	  op1.val.n = rbp;
	  	  op1.t = NUM;
	  ...
	  case RCE:/*Verifica o tipo do topo na pilha de excecucao e recupera a
	    ...
	    break;
	  case EQ:/*Desempilha os 2 objetos do topo, verifica a igualdade entre
	  ...
	  case GT:/*Desempilha os 2 objetos do topo, verifica se um e estritamente
	    ...
	    break;
	  case GE:/*Desempilha os 2 objetos do topo, verifica se um e maior ou
	  			igual ao outro e empilha um operando verdadeiro ou falso*/
	    op1 = desempilha(pil);
	    op2 = desempilha(pil);
	  ...
	  case LT:/*Desempilha os 2 objetos do topo, verifica se um e estritamente
	    ...
	    break;
	  case LE:/*Desempilha os 2 objetos do topo, verifica se um e menor ou
	  			igual ao outro e empilha um operando verdadeiro ou falso*/
	    op1 = desempilha(pil);
	    op2 = desempilha(pil);
	  ...
	  case END:/*Encerra a excecucao das instrucoes*/
	    return;
	  case PRN:/*Verifica o tipo do objeto no topo da pilha e imprime o valor
	  			do topo da pilha e desempilha o topo, nao funciona para
	  			celulas*/
	    tmp = desempilha(pil);
	  ...
	  case ALC:/*Funcao auxiliar aloca frames na pilha de excecucao para
	    ...
	    if (arg.t == NUM)
	      exec->topo += arg.val.n;
	    break;
	  case FRE:/*Funcao auxiliar libera frames da pilha de excecucao onde
	  			estavam armazenadas as variaveis locais*/
	    if (arg.t == NUM)
	      exec->topo -= arg.val.n;
	    break;
	  ...
	  case SISM:/*System call, verifica o tipo do argumento e tenta mover a
	    ...
	    break;
	  case SISA:/*System call, verifica o tipo do argumento e tenta atacar
	  			algum inimigo*/
	    if (arg.t == NUM){
	      arg.val.n += 10;
	      tmp = Sistema(arg, m);
	  ...
	  case SISR:/*System call, verifica o tipo do argumento e tenta recolher os
	    ...
	    break;
	  case SISD:/*System call, verifica o tipo do argumento e tenta depositar
	  			os cristais na base inimiga*/
	    if (arg.t == NUM){
	  	  arg.val.n += 30;
	      tmp = Sistema(arg, m);
